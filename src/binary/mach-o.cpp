...
Result<const char*, internal_error> mach_o::symtab_info_data::get_string(
...
Result<mach_o, internal_error> mach_o::open_mach_o(
...
Result<std::reference_wrapper<optional<mach_o::symtab_info_data>>,
    internal_error>
mach_o::get_symtab_info()
{
    if(!symtab_info.has_value() && !tried_to_load_symtab)
    {
        // don't try to load the symtab again if for some reason loading here
        // fails
        ...
        for(const auto& command : load_commands)
        {
            if(command.cmd == LC_SYMTAB)
            {
                ...
                auto string =
                    load_string_table(info.symtab.stroff, info.symtab.strsize);
                if(!string)
                {
...
void mach_o::print_symbol_table_entry(const nlist_64& entry,
    const char* stringtab, std::size_t stringsize, std::size_t j) const
{
    const char* type = "";
    ...
    fprintf(stderr, "%5llu %8llx %2llx %7s %2llu %4llx %16llx %s\n", to_ull(j),
        to_ull(entry.n_un.n_strx), to_ull(entry.n_type), type,
        to_ull(entry.n_sect), to_ull(entry.n_desc), to_ull(entry.n_value),
        stringtab == nullptr             ? "Stringtab error"
        : entry.n_un.n_strx < stringsize ? stringtab + entry.n_un.n_strx
    ...
    for(const auto& command : load_commands)
    {
        if(command.cmd == LC_SYMTAB)
        {
            ...
            for(std::size_t j = 0; j < symtab.nsyms; j++)
            {
                ...
                print_symbol_table_entry(entry.unwrap_value(),
...
{
    // we have a bunch of symbols in our binary we need to pair up with symbols
    // from various .o files first collect symbols and the objects they come
    // from
    debug_map debug_map;
    ...
    for(std::size_t j = 0; j < symtab.nsyms; j++)
    {
        auto load_entry = bits == 32 ? load_symtab_entry<32>(symtab.symoff, j)
                                     : load_symtab_entry<64>(symtab.symoff, j);
        if(!load_entry)
        {
        ...
        {
            case N_SO:
                // pass - these encode path and filename for the module, if
                // applicable
                break;
            case N_OSO:
            {
            ...
            case N_FUN:
            {
                ...
                if(str.unwrap_value()[0] == 0)
                {
                    ...
                    debug_map[current_module].push_back(
                        std::move(current_function).unwrap());
                }
                else
                {
...
Result<const std::vector<mach_o::symbol_entry>&, internal_error>
mach_o::symbol_table()
{
    if(symbols)
    {
    ...
    for(std::size_t j = 0; j < symtab.nsyms; j++)
    {
        ...
        if((entry.n_type & N_TYPE) == N_SECT)
        {
            ...
            symbol_table.push_back({entry.n_value, str.unwrap_value()});
        }
    }
...
optional<std::string> mach_o::lookup_symbol(frame_ptr pc)
{
    ...
    ;
    auto it = first_less_than_or_equal(symtab.begin(), symtab.end(), pc,
    ...
    // TODO: We subtracted one from the address so name + diff won't show up in
    // the objdump, decide if desirable to have an easier offset to lookup
...
void mach_o::print_debug_map(const debug_map& debug_map)
{
    for(const auto& entry : debug_map)
    {
        ...
        {
            std::cerr << "    " << symbol.name << " " << std::hex
                      << symbol.source_address << " " << symbol.size << std::dec
                      << '\n';
        }
    }
}
...
Result<monostate, internal_error> mach_o::load_mach()
{
    ...
    bits = Bits;
    using Mach_Header = typename std::conditional<Bits == 32, mach_header,
        mach_header_64>::type;
...
Result<monostate, internal_error> mach_o::load_fat_mach()
{
    ...
    // thread_local static struct LP(mach_header)* mhp =
    // _NSGetMachExecuteHeader(); off_t arch_offset = (off_t)header_size;
    ...
    //     std::uint32_t magic = load_bytes<std::uint32_t>(file,
    //     mach_header_offset); std::cerr<<"xxx: "<<arch.cputype<<" :
    //     "<<mhp->cputype<<std::endl; std::cerr<<"     "<<arch.cpusubtype<<" :
    //     "<<static_cast<cpu_subtype_t>(mhp->cpusubtype &
    ...
    fat_arch* best = NXFindBestFatArch(
        mhp->cputype, mhp->cpusubtype, fat_arches.data(), header.nfat_arch);
    if(best)
    {
    ...
    // If this is reached... something went wrong. The cpu we're on wasn't
    // found.
    return internal_error(
        "Couldn't find appropriate architecture in fat Mach-O");
}

template <std::size_t Bits>
Result<segment_command_64, internal_error> mach_o::load_segment_command(
    std::uint32_t offset) const
{
    using Segment_Command = typename std::conditional<Bits == 32,
        segment_command, segment_command_64>::type;
    ...
    static_assert(
        sizeof common.segname == 16 && sizeof segment.segname == 16, "xx");
...
Result<symtab_command, internal_error> mach_o::load_symbol_table_command(
    std::uint32_t offset) const
{
...
template <std::size_t Bits>
Result<nlist_64, internal_error> mach_o::load_symtab_entry(
    std::uint32_t symbol_base, std::size_t index) const
{
    using Nlist = typename std::conditional<Bits == 32, struct nlist,
        struct nlist_64>::type;
...
Result<std::vector<char>, internal_error> mach_o::load_string_table(
    std::uint32_t offset, std::uint32_t byte_count) const
{
...
Result<maybe_owned<mach_o>, internal_error> open_mach_o_cached(
    const std::string& object_path)
{
    ...
    else
    {
        ...
        static std::unordered_map<std::string, Result<mach_o, internal_error>>
            cache;
        auto it = cache.find(object_path);
        if(it == cache.end())
        {
            auto res =
                cache.insert({object_path, mach_o::open_mach_o(object_path)});
            VERIFY(res.second);
            it = res.first;
        }
        return it->second.transform(
            [](mach_o& obj) { return maybe_owned<mach_o>(&obj); });
    }
}
} // namespace detail
} // namespace cpptrace
